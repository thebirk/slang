...
public class IrGfx {
	...
	public static void addFunctions(IrScope scope) {
		...
		gfx.put(new IrString("loadTexture", null), new IrJavaFunc(1, new SourceLoc("builtin-gfx-loadtexture", 1, 1)) {
			@Override
			public IrValue javaCall(IrScope scope, ArrayList<IrValue> args) {
				IrValue a = args.get(0);
				if(a.getType() != Type.STRING) {
					throw new IrException(getLocation(), "Wrong argument types passed, 'loadTexture' expect a string!");
				}
				IrString s = (IrString) a;
				return new Texture(s.getValue(), getLocation());
			}
		});

		gfx.put(new IrString("drawTexture", null), new IrJavaFunc(3, new SourceLoc("builtin-gfx-loadtexture", 1, 1)) {
			@Override
			public IrValue javaCall(IrScope scope, ArrayList<IrValue> args) {
				IrValue img = args.get(0);
				IrValue xx = args.get(1);
				IrValue yy = args.get(2);
				if(img.getType() != Type.USERDATA || xx.getType() != Type.NUMBER || yy.getType() != Type.NUMBER) {
					throw new IrException(getLocation(), "Wrong argument types passed, 'drawTexture' expect a texture and two numbers!");
				}

				IrUserdata d = (IrUserdata) img;
				if(d.getUserType() != Texture.userType) {
					throw new IrException(getLocation(), "Wrong argument type passed! First argument was not a texture!");
				}

				Texture t = (Texture) d;
				IrNumber x = (IrNumber) xx;
				IrNumber y = (IrNumber) yy;

				window.drawTexture(t, (int)x.getValue(), (int)y.getValue());
